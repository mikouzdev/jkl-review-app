...
import { userActionLimiter } from "../utils/limiters.js";

const router = express.Router();

// admin endpoint to get all reviews in a descending order latest first.
...
router.delete("/:id", authenticate, userActionLimiter, async (req: AuthenticatedRequest, res: Response) => {
...
router.put("/:id", authenticate, userActionLimiter, async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user?.userId;
  const reviewId = Number(req.params.id);
  ...
  if (review.comment && review.comment?.length > MAX_COMMENT_LENGTH)
    return res.status(400).send({ error: "comment is too long" });
...
router.get("/me", authenticate, userActionLimiter, async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user?.userId;
